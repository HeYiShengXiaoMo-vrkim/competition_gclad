...
parser.add_argument('--layers', type=int, default=2, help='Number of Basic-model layers.')
...
for layer in range(args.layers):
    ...
    for epoch in epoch_iter:
        ...
        loss = F.nll_loss(F.log_softmax(logits[train_mask], 1), labels[train_mask], reduction='none') \
               + args.att_loss_weight * F.nll_loss(F.log_softmax(logits_GAT[train_mask], 1), labels[train_mask],
                                                   reduction='none')
